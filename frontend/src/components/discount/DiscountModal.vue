<template>
    <div>
        <b-modal id="discountModal" no-close-on-backdrop
            ...
            <template v-slot:default>
                <ValidationObserver>
                    <b-form slot-scope="{ validate }" @submit.prevent="validate().then(handleSubmit)"
                        ...
                        <ValidationProvider rules="required|integer|min_value:1|max_value:100"
        ...
        methods: {
            ...
            handleSubmit() {
                if (this.formDiscount.id < 0)
                    ...
                    this.$emit('updateDiscount', this.formDiscount);
            }
        },
        computed: {
            actionMessage() {
