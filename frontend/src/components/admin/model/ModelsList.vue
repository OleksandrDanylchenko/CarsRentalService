            ...
            <b-row>
              <b-col class="d-flex align-items-center justify-content-between">
                ...
                <b-button
                  ...
                  @click="openModelModal(-1)"
                >
                  <i class="fas fa-plus-circle"></i>&nbsp;Додати нову модель
                  авто
                </b-button>
              </b-col>
            </b-row>
            <b-card-group columns>
              <div v-for="model in models" :key="model.id">
                <div v-if="model.imageName != null">
                  <b-card
                    :img-src="
                      require('@/assets/modelsPhotos/' + model.imageName)
                    "
...
<script>
  ...
  methods: {
    ...
    addModel(addForm) {
      DataService.addRecord(this.resource, addForm)
        .then(() => {
        ...
        .catch((error) => {
          ...
          if (error.response.status === 409) {
            this.addError(
              `Таблиця уже містить модель ${
                JSON.parse(error.response.config.data).model
              }`
            );
          } else {
            this.addError(
              `Нова модель містить інформацію, що суперечить обмеженням`
            );
          }
        });
    ...
    updateModel(updateForm) {
      DataService.updateRecord(this.resource, updateForm)
        .then(() => {
          this.addMessage(
            `Модель №${JSON.parse(updateForm.get("model")).id} змінено успішно`
          );
          this.refreshModels();
        })
        .catch((error) => {
          ...
          if (error.response.status === 409) {
            ...
            this.addError(
              `Змінювана модель містить інформацію, що суперечить обмеженням`
            );
          }
        });
      this.$bvModal.hide("modelModal");
