<template>
  <div>
          ...
          <b-row>
            <b-col class="d-flex align-items-center justify-content-between">
              ...
              <b-button
                ...
                @click="openSpecificationModal(-1)"
              >
          ...
          >
            ...
            <template v-slot:cell(editModal)="data">
              <b-button
                ...
                @click="openSpecificationModal(data.item.id)"
              >
    ...
    <DeleteModal
      :processingId="processingId"
      @deleteRecord="deleteSpecification"
    />
  </div>
</template>
...
<script>
  ...
  data() {
    return {
      fields: [
        ...
        {
          key: "fuelConsumption",
          label: "Споживання пального (л./100км.)",
          sortable: true,
        },
  ...
  methods: {
    ...
    addSpecification(newSpecification) {
      this.isBusy = true;
      DataService.addRecord(this.resource, newSpecification)
        .then(() => {
        ...
        .catch((error) => {
          console.log(error);
          this.addError(
            `Нова конфігурація містить інформацію, що суперечить обмеженням`
          );
        });
      this.isBusy = false;
    ...
    updateSpecification(updateSpecification) {
      this.isBusy = true;
      DataService.updateRecord(this.resource, updateSpecification)
        .then(() => {
          this.addMessage(
            `Конфігурацію №${updateSpecification.id} змінено успішно`
          );
          this.refreshSpecifications();
        })
        .catch((error) => {
          console.log(error);
          this.addError(
            `Змінювана конфігурація містить інформацію, що суперечить обмеженням`
          );
        });
      this.isBusy = false;
      this.$bvModal.hide("specificationModal");
