    ...
    >
      ...
      <template v-slot:default>
        <ValidationObserver>
          ...
          >
            <ValidationProvider
              rules="required|decimal:1|min_value:0.8|max_value:6|min:3"
            ...
            <ValidationProvider
              rules="required|integer|min_value:50|max_value:2000"
            ...
            <ValidationProvider rules="required" name="з типом пального">
              <b-form-group slot-scope="{ valid, errors }">
                <b-input-group prepend="Тип пального">
                  <b-form-select
                    v-model="formSpecification.fuelType"
                    :options="fuelTypes"
                    :state="errors[0] ? false : valid ? true : null"
                  />
                  <b-form-invalid-feedback>
            ...
            <ValidationProvider
              rules="required|decimal:1|min_value:3|max_value:50|min:3"
            ...
            <ValidationProvider rules="required" name="з типом трансмісії">
              <b-form-group slot-scope="{ valid, errors }">
                <b-input-group prepend="Тип трансмісії">
                  <b-form-select
                    v-model="formSpecification.transmissionType"
...
<script>
  ...
  methods: {
    loadValues() {
      this.$nextTick(() => {
        DataService.retrieveRecord(this.resource, this.processingId)
          .then((response) => {
            this.formSpecification.engineCapacity =
              response.data.engineCapacity;
            this.formSpecification.horsepowers = response.data.horsepowers;
            this.formSpecification.fuelType = response.data.fuelType;
            this.formSpecification.fuelConsumption =
              response.data.fuelConsumption;
            this.formSpecification.transmissionType =
              response.data.transmissionType;
          })
          .catch((error) => {
            console.log(error);
    ...
    handleSubmit() {
      ...
      else this.$emit("updateSpecification", this.formSpecification);
    },
  },
  computed: {
    actionMessage() {
      return this.processingId <= 0
        ? "Додайте нову конфігурацію авто"
        : "Змініть конфігурацію авто";
    },
    action() {
