...
<script>
  ...
  methods: {
    ...
    addRent(newRent) {
      this.isBusy = true;
      DataService.addRecord(this.resource, newRent)
        .then(() => {
        ...
        .catch((error) => {
          console.log(error);
          this.addError(
            `Новий запис оренди містить інформацію, що суперечить обмеженням`
          );
        });
      this.isBusy = false;
    ...
    updateRent(updateRent) {
      this.isBusy = true;
      DataService.updateRecord(this.resource, updateRent)
        .then(() => {
        ...
        .catch((error) => {
          console.log(error);
          this.addError(
            `Змінюваний запис оренди містить інформацію, що суперечить обмеженням`
          );
        });
      this.isBusy = false;
      this.$bvModal.hide("rentModal");
