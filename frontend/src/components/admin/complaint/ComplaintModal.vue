<template>
    <div>
        <b-modal id="complaintModal" no-close-on-backdrop
                        ...
                        <ValidationProvider rules="required" name="з записом про оренду">
                            <b-form-group slot-scope="{ valid, errors }">
                                <b-input-group prepend="Запис про оренду">
                                    <b-form-select v-model="formComplaint.rent.id" :options="availableRents"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            loadChoices() {
                DataService.retrieveAllRecords(this.rentsResource).then(response => {
                    response.data.forEach(record => {
                        let rentOption = {
                            value: record.id,
                            text: '№' + record.id + ' ' + record.client.firstName + ' ' + record.client.secondName
                        };
                    ...
                    console.log(error);
                    if (error.response.status === 404)
                        this.$emit('addError', `Таблиця записів про оренду не містить записів, тому не можна створити скаргу.`);
                    else
                        this.$emit('addError', `Модельне вікно спричинило помилку ${error}`);
                    this.$bvModal.hide("complaintModal");
                });
            ...
            handleSubmit() {
                if (this.formComplaint.id < 0)
                    ...
                    this.$emit('updateComplaint', this.formComplaint);
            }
        },
        computed: {
            actionMessage() {
