        ...
        <b-col>
          ...
          >
            ...
            <template v-slot:cell(rentClient)="data">
              {{
                data.item.rent.client.firstName +
                " " +
                data.item.rent.client.secondName
              }}
            </template>
            <template v-slot:cell(rentCar)="data">
              {{
                data.item.rent.car.model.brand +
                " " +
                data.item.rent.car.model.model
              }}
            </template>
            ...
            <template v-slot:cell(editModal)="data">
              <b-button
                ...
                @click="openComplaintModal(data.item.id)"
              >
...
<script>
  ...
  methods: {
    ...
    addComplaint(newComplaint) {
      this.isBusy = true;
      DataService.addRecord(this.resource, newComplaint)
        .then(() => {
        ...
        .catch((error) => {
          console.log(error);
          this.addError(
            `Нова скарга містить інформацію, що суперечить обмеженням`
          );
        });
      this.isBusy = false;
    ...
    updateComplaint(updateComplaint) {
      this.isBusy = true;
      DataService.updateRecord(this.resource, updateComplaint)
        .then(() => {
        ...
        .catch((error) => {
          console.log(error);
          this.addError(
            `Змінювана скарга містить інформацію, що суперечить обмеженням`
          );
        });
      this.isBusy = false;
      this.$bvModal.hide("complaintModal");
