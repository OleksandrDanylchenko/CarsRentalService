      ...
      <template v-slot:default>
        <ValidationObserver>
          ...
          >
            ...
            <ValidationProvider rules="required" name="з конфігурацією">
              <b-form-group slot-scope="{ valid, errors }">
                <b-input-group prepend="Номер конфігурації авто">
                  <b-form-select
                    v-model="formCar.specification.id"
            ...
            <ValidationProvider
              rules="required|integer|min_value:50|max_value:400000"
            ...
            <ValidationProvider rules="required">
              <b-form-group slot-scope="{ valid, errors }">
                <b-input-group
                  prepend="Доступний для оренди"
                  class="d-flex align-items-center"
                >
                  <b-form-radio
                    v-model="formCar.rentable"
                    name="rentable-radio"
                    value="true"
                    class="ml-2"
                  >
...
<script>
  ...
  methods: {
    ...
    loadChoices() {
      DataService.retrieveAllRecords(this.modelsResource)
        .then((response) => {
        ...
        .catch((error) => {
          ...
          if (error.response.status === 404)
            this.$emit(
              "addError",
              `Таблиця моделей не містить записів, тому не можна створити авто.`
            );
          else
            this.$emit(
              "addError",
      ...
      DataService.retrieveAllRecords(this.specificationsResource)
        .then((response) => {
          response.data.forEach((record) => {
      ...
      DataService.retrieveAllRecords(this.centersResource)
        ...
        .catch((error) => {
          ...
          if (error.response.status === 404)
            this.$emit(
              "addError",
              `Таблиця центрів оренди не містить записів, тому не можна створити авто.`
            );
          else
            this.$emit(
              "addError",
    ...
    handleSubmit() {
      if (this.formCar.id < 0) this.$emit("addCar", this.formCar);
      else this.$emit("updateCar", this.formCar);
    },
  },
  computed: {
    actionMessage() {
