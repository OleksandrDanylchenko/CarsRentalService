<template>
    <div>
        ...
        <div class="mt-4">
            ...
            <b-row>
                <b-col>
                    ...
                    <b-table id="carsTable" hover :items="cars" :fields="fields"
                        ...
                        <template v-slot:cell(specFields)="data">
                            <b-button variant="outline-dark" v-b-toggle="'specs' + data.item.id">Деталі</b-button>
                            <b-collapse :id="'specs' + data.item.id" class="mt-2">
                                <b-card>
                                    <p>Об'єм двигуна: {{ data.item.specification.engineCapacity}}л.</p>
                                    <p>Кінські сили: {{ data.item.specification.horsepowers}}к.с.</p>
                                    <p>Тип пального: {{ data.item.specification.fuelType}}</p>
                                    <p>Споживання пального: {{ data.item.specification.fuelConsumption}}л./100км.</p>
                                    <p>Тип трансмісії: {{ data.item.specification.transmissionType}}</p>
                                </b-card>
                            </b-collapse>
                        </template>
                        ...
                        <template v-slot:cell(editModal)="data">
                            <b-button pill variant="outline-dark" @click="openCarModal(data.item.id)">
        ...
        <CarModal :processingId="processingId" @addCar="addCar"
                  @updateCar="updateCar" @addError="addError"/>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            updateCar(updateCar) {
                this.isBusy = true;
                DataService.updateRecord(this.resource, updateCar).then(() => {
