            ...
            <b-card-group columns>
                <div v-for="car in popularCars" :key="car.id">
                    <div v-if="car.model.imageName != null">
                        <b-card :img-src="require('@/assets/modelsPhotos/' + car.model.imageName)"
                                :img-alt="car.model.model + ' photo'" img-top
                                class="fadeInUp" v-wow>
        ...
        methods: {
            loadCars() {
                DataService.retrieveAllRecords(this.carsResource).then(response => {
                    this.popularCars = this._.uniqBy(response.data, "model.model").slice(0, 3);
                }).catch(error => {
                    console.log(error);
                });
            }
